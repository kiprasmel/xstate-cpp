...
Interpreter Interpreter::onStart(const std::function<void()> callback) {
	this->handleOnStart = [=](Interpreter self) { callback(); };
	return (*this);
}

Interpreter Interpreter::onStart(const std::function<void(Interpreter self)> callback) {
	this->handleOnStart = [=](Interpreter self) { callback((self)); };
	return (*this);
}
...
Interpreter Interpreter::onTransition(const std::function<void(Interpreter self)> callback) {
...
Interpreter Interpreter::onStop(const std::function<void()> callback) {
	this->handleOnStop = [=](Interpreter self) { callback(); };
	return (*this);
}

Interpreter Interpreter::onStop(const std::function<void(Interpreter self)> callback) {
	this->handleOnStop = [=](Interpreter self) { callback(self); };
	return (*this);
}
