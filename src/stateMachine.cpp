...
StateMachineState StateMachine::transition(std::string currentState, std::string event) {
	...
	/**
	 * do NOT assign / initialize `head` to `this` here!
	 *
	 * in that case, when `stateAccessTokens.size() >= 2`,
	 * we'd end up modifying `this`.
	 *
	*/
	StateMachine* head;
	...
	if (stateAccessTokens.size() <= 1) {
		...
		/**
		 * point `head` to the next state machine
		 *
		 * we must initialize before traversing deeper,
		 * because otherwise the assignment
		 * `head = &head->states[stateToken]`
		 * would mean that we're accessing `&head->states[stateToken]`,
		 * while `head` is not initialized!
		 *
		 * This means that we accessed the first token.
		 * Thus, in the `for` loop, instead of starting off `i` from `0`,
		 * we'll start it off from `1`
		 *
		*/
	...
	*/
	...
	/** TODO - this probably should not even exist yo */
	...
	if (nextStateIt != head->states[lastStateAccessToken].on.end()) {
		/**
		 * if a new state was found based on the current state + event,
		 * update current state with the new state.
		*/

		if (head->states.find(nextStateIt->second) == head->states.end()) {
			std::cerr << "\nERR: next state's target does NOT exist inside state machine's `states`"
				/** TODO better message */
				<< "\ninfo:"
					<< "\n3. args to the `transition` function:"
					<< "\nprovided `currentState`: " << currentState
			...
			/** it's now safe & appropriate to update the state */
			head->state.value = nextStateIt->second;
		}
	}
